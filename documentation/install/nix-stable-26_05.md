# Instalation:
## Geting the Repo
1. Clone the nix-stable-26.05 Branch in your home directory. This is required, as the flake will simlink some of the dotfiles from the `.config` directory to their corsipoding dotfiles in `nixConfig/Dotfiles`. Those simlinks will fail if the flake is not located in the correct spot.
```
cd ~
