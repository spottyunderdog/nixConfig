# Instalation:
## Geting the Repo
1. Clone the nix-unstable-omniflake Branch and enter repoin your home directory. This is required, as the flake will simlink some of the dotfiles from the `.config` directory to their corsipoding dotfiles in `nixConfig/Dotfiles`. Those simlinks will fail if the flake is not located in the correct spot.
```
cd ~
...
4. If your making more then one user account make sure that the `uuid` variable is different for each user you have on a host. This means that users on your host and located in the `nicConfig/modules/sharedSystemConfigs/users` dir must all have different uuids. 
...
2. Next, you will need to configure your monitors, run `hyprctl monitors` and make note of your monitor outputs for your displays ID (Ie `eDP-1` or `HDMI-A-1`). Then within the repo navigate to `nixConfig/dotFiles/hypr/config` and look for the `variables.lua` & `monitors.lua` file. In `variables.lua` set the values of the monitor variables you got to match your preffered display layout. Then you can configure your monitor settings in `monitors.lua` I also include the `hyprmon` package if you would like a tui way of controling your monitors settings.
3. If you have an nvida GPU you can find envirment variables related to them at `nixConfig/dotFiles/uwsm/env`(For UWSM managed sessions) and `nixConfig/hypr/config/environment.lua`. Uncomment the commented out lines, and they will ensure your Envirment is properly set up for NVIDIA based GPUs.
...
1. Open the dot files directory, located at `nixConfig/dotFiles`. Then delete the contents of the `hypr/` directory, and if your using UWSM delete the contents of the `uwsm/` directory as well. 
...
##### Replacing My dotfiles with your own.
1. Open the dot files directory, located at `nixConfig/dotFiles`. Then delete the contents of the `niri/` directory
