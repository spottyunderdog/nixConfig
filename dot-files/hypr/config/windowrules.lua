...
hl.window_rule({ match = { class = "^(.*[Ll]auncher.*)$" }, float = true, workspace = LAUNCHER_WORKSPACE })
hl.window_rule({ match = { class = "^(vesktop|discord)$" }, workspace = COMS_WORKSPACE }) -- Spawn Instances Of Vesktop and Discord on the 16th workspace
hl.window_rule({ match = { title = "^(Steam|Heroic.*)" }, workspace = LAUNCHER_WORKSPACE}) -- Spawn instances of steam & heroic in the game launcher workspace
...
-- Hide Apps from screensharing
local hiddenApps = {
	{ title = "^(Proton .*)" },
	{ title = "^(.*OBS.*)" },
	{ title = "^(.*[Mm]ail.*|.*[Ii]nbox.*)" },
	{ title = "^(.*WhatsApp.*)" },
	{ class = "thunderbird" }
}
for _, m in ipairs(hiddenApps) do hl.window_rule({match = m, no_screen_share = true }) end
